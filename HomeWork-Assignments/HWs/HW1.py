...
def app():
    ...
    if uploaded_file and question:
        ...
        quality_raw = {"gpt-3.5-turbo": 1, "gpt-4.1": 3, "gpt-5": 4, "gpt-5-nano": 2}
        latencies = [results[m]["latency"] for m in models]
        costs     = [results[m]["cost"]    for m in models]

        for m in models:
            r = results[m]
            r["score_quality"] = (quality_raw[m] - 1) / 3.0
        ...
        best_r = results[best]
        contribs = {"quality": best_r["contrib_quality"], "speed": best_r["contrib_speed"], "cost": best_r["contrib_cost"]}
